<template>
    <AppLayout title="Editar Ruta" :breadcrumbs="breadcrumbs">
        <Head :title="`Editar: ${routeData.name}`" />
        ...
        <div class="mb-6">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl shadow-xl p-6 mb-6 border border-emerald-200 dark:border-emerald-800 ring-1 ring-gray-900/5 dark:ring-white/10">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex-1">
                            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar Ruta</h1>
                            <p class="text-gray-600 dark:text-gray-300 mt-1">{{ routeData.code }} - {{ routeData.name }}</p>
                        </div>
                        <Link
...
onMounted(() => {
    ...
    form.name = props.routeData.name
    form.description = props.routeData.description || ''
    form.type = props.routeData.type
    form.collection_points = props.routeData.collection_points.map((point, index) => ({
        lat: point.lat,
        lng: point.lng,
        address: point.address,
        priority: point.priority || index + 1
    }))
    form.start_time = props.routeData.start_time || ''
    form.end_time = props.routeData.end_time || ''
    form.days_of_week = props.routeData.days_of_week || []
    form.assigned_vehicle_id = props.routeData.assigned_vehicle_id
})
...
const submit = () => {
    form.put(route('routes.update', props.routeData.id))
}
</script>
